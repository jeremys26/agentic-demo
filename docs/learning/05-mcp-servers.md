...
- `mcp_server/registry.py` — the config-driven tool registry. Each tool module registers its handlers via a `@register(name, service, kind)` decorator; `registry.all_tools()` returns every registered `ToolSpec`. This is the "one place that knows about every tool" the earlier draft of this doc anticipated — `main.py` loops over it once to wire every tool into both the MCP server and the `/tools` REST introspection endpoint, so the two can't drift apart the way two independently-maintained lists could.
...
An earlier version of this doc flagged a real gap: `ProposedAction.status` could theoretically be `approved`/`rejected`, but nothing ever set it — so `pending_approval` actions could be created and listed but never resolved. That's fixed now:

- `POST /agent-actions/{id}/approve` and `POST /agent-actions/{id}/reject` on the MCP server (`main.py`) — plain REST, **deliberately not MCP tools**. An agent must never be able to approve its own proposed action; only a human, via this endpoint (which React-Admin will call), can. `guardrails/engine.py`'s `resolve_proposed_action()` backs both: it updates the **original** `AgentToolCall` row (`rejected` or `approved_executed`) rather than inserting a second log line, so the Agent Actions list doesn't keep a stale pending row with live Approve/Reject buttons after the human has already decided.
- The real execution logic — the SmartSpot360 "apply" call, the Captivator360 create-then-resolve call — was pulled out into shared `_execute_reallocate_budget()` / `_execute_request_creative_refresh()` helpers in `engine.py`, used by **both** the `auto_execute` path and the newly-added approve path. This was a deliberate lesson from the earlier maestro360 file-corruption incident (a rogue background-builder fork wrote a divergent, broken version of one file): don't let two code paths that are supposed to do the same thing risk drifting apart by being written twice.
- Idempotency is handled at the top of `resolve_proposed_action()`: if a `ProposedAction` isn't currently `pending_approval` (already approved, already rejected, or doesn't exist), it returns a clean error instead of attempting anything.

**One low-priority edge case worth knowing about, not urgent to fix**: `_execute_request_creative_refresh()` is a two-step remote call (create the refresh request, then resolve it as approved). If the *first* HTTP call succeeds but the *second* fails partway through — a network blip, a service restart — the `ProposedAction` stays `pending_approval` (so a human can retry), but retrying calls the same two-step function again, which would create a **second** `CreativeRefreshRequest` rather than resuming the first. `_execute_reallocate_budget()` doesn't have this problem, since it checks `recommendation["applied"]` before doing anything, making it safe to retry. Low probability in practice (both calls hit the same Docker-internal network in well under a second), and not worth solving for a Tier 1 demo, but the fix — if it's ever needed — would be having `_execute_request_creative_refresh` check for an existing non-rejected `CreativeRefreshRequest` for that creative before creating a new one, the same way the budget path already guards on `applied`.
